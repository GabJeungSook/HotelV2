...
class GhostCheckinGuardTest extends TestCase
{
    ...
    /** @test */
    public function kiosk_render_hides_rooms_with_unresolved_previous_guest()
    {
        // A room with an open (is_check_out=0) CheckinDetail must NOT appear
        // in the kiosk's available rooms list. The confirm-time guard remains
        // as defense, but users shouldn't be presented with such rooms at all.
        [$branch, $floor, $type, $rate, $room, $user] = $this->seedScaffolding();

        $unresolvedGuest = Guest::create([
            'branch_id' => $branch->id,
            'name' => 'Unresolved Display Test',
            'qr_code' => 'TEST-HIDE-KIOSK-001',
            'room_id' => $room->id,
            'rate_id' => $rate->id,
            'type_id' => $type->id,
            'static_amount' => 300,
        ...
        $this->assertFalse(
            $rooms->contains('id', $room->id),
            'Kiosk must not display a room whose previous guest has not checked out.'
        );
    }

    /** @test */
    public function roomboy_index_render_hides_rooms_with_unresolved_previous_guest()
    {
        [$branch, $floor, $type, $rate, $room, $user] = $this->seedScaffolding();

        $unresolvedGuest = Guest::create([
            'branch_id' => $branch->id,
            'name' => 'Unresolved Display Roomboy',
            'qr_code' => 'TEST-HIDE-ROOMBOY-001',
            'room_id' => $room->id,
            'rate_id' => $rate->id,
            'type_id' => $type->id,
            'static_amount' => 300,
        ...
        $assigned = $component->viewData('assignedRooms');
        $unassigned = $component->viewData('unassignedRooms');

        $this->assertFalse(
            $assigned->contains('id', $room->id),
            'Roomboy assigned list must not include a room whose previous guest has not checked out.'
        );
        $this->assertFalse(
            $unassigned->contains('id', $room->id),
            'Roomboy unassigned list must not include a room whose previous guest has not checked out.'
        );
    }

    /** @test */
    public function kiosk_render_shows_rooms_where_previous_guest_checked_out()
    {
        // Happy path for the display filter: a properly-closed CheckinDetail
        // does NOT hide the room.
        [$branch, $floor, $type, $rate, $room, $user] = $this->seedScaffolding();

        $pastGuest = Guest::create([
            'branch_id' => $branch->id,
            'name' => 'Past Guest (checked out)',
            'qr_code' => 'TEST-HAPPY-002',
            'room_id' => $room->id,
            'rate_id' => $rate->id,
            'type_id' => $type->id,
            'static_amount' => 300,
        ...
        $this->assertTrue(
            $rooms->contains('id', $room->id),
            'Kiosk should display the room once the previous guest is properly checked out.'
        );
    }
