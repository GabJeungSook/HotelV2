...
use App\Http\Livewire\Frontdesk\Monitoring\ExtendGuest;
...
class ExtendGuestTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function component_mounts_with_guest()
    {
        ...
        $component = Livewire::actingAs($this->user)
            ->test(ExtendGuest::class, ['record' => $guest->id]);

        $component->assertSet('guest.id', $guest->id);
        $component->assertSet('rate.room_id', $this->room->id);
    }

    /** @test */
    public function extension_uses_extension_rate_within_cycle()
    {
        $guest = $this->createGuestWithCheckin([
            'number_of_hours' => 12,
            'next_extension_is_original' => false,
        ...
        $component->set('extension_rate_id', $this->extRate6->id);

        $component->assertSet('total_amount', 100);
    }

    /** @test */
    public function extension_uses_original_rate_after_cycle_reset()
    {
        $guest = $this->createGuestWithCheckin([
            'number_of_hours' => 0,
        ...
        $component->assertSet('total_amount', 250);
    }

    /** @test */
    public function extension_rate_lookup_uses_room_id()
    {
        ...
        // Guest is in room 1, so should use room 1's 6hr rate (250), not room 2's (500)
        $component->assertSet('total_amount', 250);
    }
}
